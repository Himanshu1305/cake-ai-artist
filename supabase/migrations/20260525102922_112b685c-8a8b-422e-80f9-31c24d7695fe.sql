CREATE OR REPLACE FUNCTION public.get_public_cake(p_id uuid)
 RETURNS TABLE(id uuid, image_url text, recipient_name text, message text, occasion_type text, audio_url text, audio_duration_seconds integer, created_at timestamp with time zone, sender_name text, share_group_id uuid, sibling_image_urls text[])
 LANGUAGE sql
...
AS $function$
  SELECT
    ...
    COALESCE(
      ...
      (
        SELECT s.audio_duration_seconds
        FROM public.generated_images s
        WHERE s.share_group_id = gi.share_group_id
          AND gi.share_group_id IS NOT NULL
          AND s.audio_url IS NOT NULL
