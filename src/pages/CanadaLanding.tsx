  ...
  const loadFeaturedCakes = async () => {
    try {
      const { data, error } = await supabase
        ...
        .or('featured_pages.cs.{"home"},featured_pages.cs.{"canada"},featured_pages.cs.{"wedding"},featured_pages.cs.{"anniversary"}')
    ...
    <div className="min-h-screen bg-gradient-celebration relative overflow-hidden">
        ...
        <div className="container mx-auto max-w-5xl">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-5">
            {[
              ...
              { emoji: "🌙", name: "Eid Cakes", desc: "Beautiful Eid Mubarak cake designs for Canada's Muslim community", href: "/eid-cake-ideas" },
              { emoji: "🍁", name: "Canada Day Cakes", desc: "Celebrate Canada Day with a maple leaf themed AI cake", href: "/free-ai-cake-designer?occasion=canada+day" },
            ].map((item) => (
                ...
                <CarouselItem key={index} className="md:basis-1/2 lg:basis-1/3">
                  <div className="p-2">
                    <div className="relative group overflow-hidden rounded-xl border-2 border-gold/30 hover:border-gold transition-all cursor-pointer" onClick={() => setSelectedCarouselImage(cake)}>
                      <img loading="lazy" decoding="async" src={resolveImageUrl(cake.image_url)} alt={cake.prompt ? `Personalized AI cake design Canada — ${cake.prompt}` : "Personalized AI-designed cake by Cake AI Artist Canada user"} className="w-full h-64 object-cover transition-transform duration-500 hover:scale-110" />
        ...
        <DialogContent className="max-w-4xl">
          {selectedCarouselImage && (
            <div className="space-y-4">
              <img loading="lazy" decoding="async" src={resolveImageUrl(selectedCarouselImage.image_url)} alt={selectedCarouselImage.prompt ? `Personalized AI cake — ${selectedCarouselImage.prompt}` : "Personalized AI-designed celebration cake — Cake AI Artist Canada"} className="w-full h-auto rounded-lg" />
      ...
      <section id="plans" className="py-16 bg-gradient-to-b from-muted/30 to-background">
        <div className="container mx-auto px-4">
          ...
          <PricingPlans country="CA" />
        </div>
      </section>

      {/* AEO Definition Box */}
      <div className="container mx-auto px-4">
        <DefinitionBox term="an AI cake generator" definition={<>An AI cake generator is an online tool that turns a name, occasion and style into a photorealistic cake design in seconds, so you can visualise a cake before ordering or baking it. Cake AI Artist returns three views of each design in about 30 seconds, spells any name correctly, and gives Canadian users 5 free designs to start.</>} />
      </div>
